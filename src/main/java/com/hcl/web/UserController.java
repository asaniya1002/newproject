...
import com.hcl.payload.JWTLoginSuccessResponse;
...
//
    ...
    public ResponseEntity<?> registerUser(@Valid @RequestBody User user, BindingResult result){
        // Validate passwords match and passowrd > 6
        userValidator.validate(user, result);

        //validate bean annotations and errors from userValidator also caught in BindingResult
        ResponseEntity<?> errorMap = mapErrorValidationService.MapValidationService(result);
        if(errorMap != null)return errorMap;
